...
int main(int argc, char** argv) {
  ...
  // clang-format off
  // RANK0: R[0]T[{{[0-9]*}}][Info] run_test(void*, int, ompi_datatype_t*)[0x{{.*}}] at {{(/.*)*}}/Util.hpp:{{[0-9?]*}}: MPI_Send: successfully checked send-buffer 0x{{.*}} of type [1 x double[16]] against 16 elements of MPI type "MPI_DOUBLE"
  // RANK1: R[1]T[{{[0-9]*}}][Info] run_test(void*, int, ompi_datatype_t*)[0x{{.*}}] at {{(/.*)*}}/Util.hpp:{{[0-9?]*}}: MPI_Recv: successfully checked recv-buffer 0x{{.*}} of type [1 x double[16]] against 16 elements of MPI type "MPI_DOUBLE"
