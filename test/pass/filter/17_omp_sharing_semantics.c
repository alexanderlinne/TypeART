...
void bar(int count) {
  ...
  // check-inst: call void @typeart_tracker_alloc_stack_omp(i8* [[POINTER1]], i32 {{[0-9]*}}, i64 1)
...
void bar2(int count) {
  ...
  // check-inst: call void @typeart_tracker_alloc_stack(i8* [[POINTER2]], i32 {{[0-9]*}}, i64 1)
  int d = 3;
  int e = 4;
  // check-inst: define {{.*}} @.omp_outlined
  // check-inst: [[ALLOCA3:%d[0-9]]] = alloca i32
  // check-inst: [[POINTER3:%[0-9a-z]+]] = bitcast i32* [[ALLOCA3]] to i8*
  // check-inst: call void @typeart_tracker_alloc_stack_omp(i8* [[POINTER3]], i32 {{[0-9]*}}, i64 1)
#pragma omp parallel for schedule(dynamic, 1) lastprivate(d) shared(e)
  for (int i = 0; i < count; ++i) {
    // Analysis should not filter d, but e...
    MPI_Send((void*)&d, e);
...
void foo_bar(int count) {
  ...
  // check-inst: call void @typeart_tracker_alloc_stack_omp(i8* [[POINTER4]], i32 {{[0-9]*}}, i64 1)
